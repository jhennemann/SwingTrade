...
Historical pullback-strategy backtest using cached OHLCV data.
Mirrors backtest_breakout.py structure for consistent comparison.
...
OUTPUT_TRADES = Path("pullback_trades.csv")
OUTPUT_SUMMARY = Path("pullback_summary.csv")
OUTPUT_EQUITY = Path("pullback_equity_curve.csv")
...
def detect_pullback_signals(
    ...
    print(f"\nDetecting pullback signals for {scan_year}...")
    ...
    for i, ticker in enumerate(tickers, 1):
        ...
        try:
            ...
            for idx, row in signal_rows.iterrows():
                ...
                # Only take signals within 15% of 52-week high
                historical = df[df.index.date <= signal_date]
                if len(historical) < 252:
                    continue
                high_52w = historical["Close"].iloc[-252:].max()
...
def main():
    print("=" * 60)
    print("  Pullback Strategy Backtest")
    print("=" * 60 + "\n")
    ...
    for year in SCAN_YEARS:
        all_signals.extend(detect_pullback_signals(cache, tickers, year, setup))

    print(f"\nTotal pullback signals: {len(all_signals)}")
    if not all_signals:
